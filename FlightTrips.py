...
if __name__ == "__main__":
    ...
    trip_report = df1.join(df2, df1.flight_number == df2.flight_number, how='inner').select('df1.flight_number',
                                                                                            col('df1.count').alias(
                                                                                                "total_trips"),
                                                                                            col(
                                                                                                'df2.sum(distance)').alias(
                                                                                                "total_distance"))

    trip_report.createOrReplaceTempView("trip_report_temp")

    sqlContext.sql(
        "insert into table refined_airlines.trip_reports select flight_number,total_trips,total_distance from trip_report_temp")
